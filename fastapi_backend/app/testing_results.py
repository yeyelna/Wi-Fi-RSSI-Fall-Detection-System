...
USER_FRIENDLY_NOT_FOUND = "File not found in official outer-test results. Please choose a testing file generated by the Evaluation script."
...
class OfficialTestingStore:
    ...
    def lookup(self, filename: str, input_mode: str = "OFFICIAL TEST LOOKUP") -> Dict:
        ...
        record = {
            "id": str(uuid.uuid4()),
            **ts,
            "file_name": str(row.get("File", filename)),
            "filename": str(row.get("File", filename)),
            "base_group": row.get("BaseGroup"),
            "outer_fold": int(row.get("OuterFold")) if pd.notna(row.get("OuterFold")) else None,
