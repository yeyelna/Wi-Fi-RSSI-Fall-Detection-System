...
// Render: const API_BASE_URL = "http://127.0.0.1:8000";
...
function friendlyErrorMessage() {
    return "File not found in official outer-test results. Please choose a testing file generated by the Evaluation script.";
}
...
function getNonFallConfidence(data) {
    if (!data) return null;
    if (data.non_fall_confidence_percent !== undefined && data.non_fall_confidence_percent !== null) {
        return data.non_fall_confidence_percent;
    }
    const fallValue = getFallConfidence(data);
    if (fallValue === null || fallValue === undefined || fallValue === "") return null;
    const numeric = Number(fallValue);
...
function renderResult() {
    ...
    container.innerHTML = `
        ...
        <div class="result-context">
            ...
            Outer fold: <strong>${escapeHtml(outerFold)}</strong> · Correctness: <strong class="${correctnessClass}">${escapeHtml(correctText)}</strong>
        </div>
