...
function renderChart(values) {
    ...
    const points = cleanValues.map((value, index) => ({ x: index + 1, y: value }));
    ...
    state.chart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        title: (items) => items.length ? `Sample ${items[0].parsed.x}` : "",
                        label: (context) => `RSSI: ${Number(context.parsed.y).toFixed(2)} dBm`
                    }
                }
            },
            scales: {
